...
async def async_setup_entry(
    ...
    _LOGGER.info("Setting up Vodafone binary sensor entities for entry: %s", entry.entry_id)
    ...
    for dev_list_name in [ROUTER_PROPERTY_LAN_DEVICES, ROUTER_PROPERTY_WLAN_DEVICES]:
        ...
        _LOGGER.debug("Processing %s devices from %s for binary sensors", len(devices), dev_list_name)
        ...
        for device in devices:
            ...
            _LOGGER.debug("Creating binary sensor for device: %s (%s)", 
    ...
    _LOGGER.info("Created %s binary sensor entities from %s total devices", len(sensors), total_devices)
...
class VodafoneDeviceBinarySensor(BinarySensorEntity):
    ...
    def __init__(self, coordinator: VodafoneDeviceCoordinator, device: dict[str, Any]):
        ...
        self.name = device.get(DEVICE_PROPERTY_HOSTNAME) or device.get(DEVICE_PROPERTY_NAME) or self.mac
        ...
        _LOGGER.debug("Initialized binary sensor for %s (MAC: %s, unique_id: %s)", 
    ...
    def is_on(self) -> bool:
        ...
        _LOGGER.debug("Binary sensor %s (%s) state: %s", 
                     self._attr_name, self.mac, "ON" if is_connected else "OFF")
        return is_connected
    ...
    async def async_added_to_hass(self) -> None:
        ...
        _LOGGER.debug("Adding binary sensor %s (%s) to Home Assistant", self._attr_name, self.mac)
        self.async_on_remove(
            self.coordinator.async_add_listener(self.async_write_ha_state)
        )
        _LOGGER.debug("Registered binary sensor %s for coordinator updates", self._attr_name)